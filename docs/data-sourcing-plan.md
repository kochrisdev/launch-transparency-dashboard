...
| **NRA public registers** — Nigeria NAFDAC Greenbook (greenbook.nafdac.gov.ng); Tanzania TMDA (imis2.tmda.go.tz); Ghana FDA (verifypermit.fdaghana.gov.gh — TLS cert expired at check); Kenya PPB (no reliable public register — sporadic PDFs) | **Both feasible registers are now automated** (verified Aug 2026): NAFDAC via its server-side DataTables JSON endpoint (`fetch-nafdac.js` — plain HTTP only; the host's HTTPS hangs after handshake); TMDA via the IMIS2 JSON backend, common-name lookup + paged public search (`fetch-tmda.js`). Ghana/Kenya remain infeasible for now | Continuous but no change feeds — monthly snapshot & diff via `sourcing.yml` | Unlicensed government-register data — attribute and cache | Ground truth for `regulatory_events` NRA rows and the country map's `registered` level. Brand-vs-INN naming inconsistent; uptime patchy. First snapshots: NGA 48 portfolio registrations (22 lapsed), TZA 12 (all compliant). |
...
| Monthly (automated — `sourcing.yml`, 3rd 06:30 UTC — plus one manual step) | Global Fund Data Service pull (grants/financials); WHO PQ + EMA regulatory pull with listings watch; NAFDAC + TMDA register pulls with registration watches; PQR Tableau crosstab export (manual); MAGICapp version poll; ICTRP XML supplement. |
| Quarterly (manual sweep) | EOI check, spot-checks of the registers still without fetchers (Ghana FDA, Kenya PPB), MMV pipeline diff, Threats Map export, UNICEF/PPM price-list check, MPAG doc archive (Apr/Oct) — feeding the quarterly brief. |
