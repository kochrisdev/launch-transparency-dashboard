...
| --- | --- | --- |
| **Automated pull** (scriptable now) | ClinicalTrials.gov v2, WHO GHO OData, DHS API, World Bank API, Global Fund Data Service OData, OECD CRS SDMX, MAGICapp guideline API, OpenFDA, EMA data tables (nightly bulk), IATI Datastore (Unitaid), MAP (R package) | Fetcher scripts writing raw JSON/CSV snapshots into `sourcing/raw/<source>/`, then normalizers producing the §2 staging CSVs (see [`sourcing/README.md`](../sourcing/README.md)). **Built:** `scripts/fetch-trials.js` (trial watch with change diff) and `scripts/fetch-globalfund.js` (malaria grants + disbursements). Can run as a scheduled GitHub Action; diffs open a review issue — **never write directly to `data/products.js`** (the analyst + validator stay the gate). |
